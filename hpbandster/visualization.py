...
def default_tool_tips(result_object, learning_curves, include_run_info=False):
    ...
    for id in learning_curves.keys():
        ...
        s = [
            'id: %s' % str(id),
            'duration (sec): %f' % ((longest_run['time_stamps']['finished'] -
        ...
        if not longest_run.loss is None:
            s += [
                str(k) + "=" + str(v)
                for k, v in sorted(id2conf[id]['config'].items())
            ]
            try:
                s += [
                    str(k) + "=" + str(v)
...
def concurrent_runs_over_time(runs, num_points=512, show=False):
    ...
    data = np.array(
        [(r.time_stamps['started'], r.time_stamps['finished']) for r in runs])
    ts = np.linspace(data.min(), data.max(), 512)
    n_workers = np.array(
        [((data[:, 0] <= t) * (data[:, 1] > t)).sum() for t in ts])
...
def performance_histogram_model_vs_random(runs, id2conf, show=False):
    model_based_runs = list(
        filter(
            lambda r: id2conf[r.config_id]['config_info']['model_based_pick'],
            runs))
    random_runs = list(
        filter(
            lambda r: not id2conf[r.config_id]['config_info']['model_based_pick'],
            runs))
...
def correlation_across_budgets(results_object, show=False):
    ...
    for cid in id2conf.keys():
        ...
        for r1, r2 in itertools.combinations(runs, 2):
            ...
            loss_pairs[float(r1.budget)][float(r2.budget)].append((r1.loss,
                                                                   r2.loss))
...
def losses_over_time(runs,
    ...
    for i, b in enumerate(budgets):
        ...
        ax.scatter(
            data[b][:, 0], data[b][:, 1], color=cmap(i), label='b=%f' % b)

        ax.step(
            data[b][:, 0], np.minimum.accumulate(data[b][:, 1]), where='post')
...
def interactive_HBS_plot(learning_curves,
    ...
    if colors_floats is None:
        ...
        for i in range(num_curves):
            seed = 100 * np.abs(
                config_ids[i][0]) + 10 * config_ids[i][1] + config_ids[i][2]
            np.random.seed(seed)
    ...
    check = CheckButtons(rax, iteration_labels,
                         [True for i in range(num_iterations)])
    ...
    def hover(event):
        ...
        if event.inaxes == ax:
            active_lines = list(
                filter(lambda l: l.contains(event)[0], all_lines))
