...
class Dispatcher(object):
    ...
    def run(self):
        with self.discover_cond:
            ...
            with Pyro4.locateNS(
                    host=self.nameserver, port=self.nameserver_port) as ns:
        ...
        with self.discover_cond:
            ...
            with Pyro4.locateNS(
                    self.nameserver, port=self.nameserver_port) as ns:
    ...
    def discover_workers(self):
        ...
        while True:
            ...
            with Pyro4.locateNS(
                    host=self.nameserver, port=self.nameserver_port) as ns:
                worker_names = ns.list(prefix="hpbandster.run_%s.worker." %
                                       self.run_id)
                self.logger.debug(
    ...
    def job_runner(self):
        ...
        while True:
            ...
            self.logger.debug('DISPATCHER: starting job %s on %s' % (str(
                job.id), worker.name))
            ...
            self.logger.debug('DISPATCHER: job %s dispatched on %s' % (str(
                job.id), worker.name))
    ...
    def register_result(self, id=None, result=None):
        ...
        with self.runner_cond:
            ...
            self.logger.debug('DISPATCHER: job %s on %s finished' % (str(
                job.id), job.worker_name))
