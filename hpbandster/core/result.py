...
def extract_HBS_learning_curves(runs):
    ...
    lc = list(
        filter(lambda t: not t[1] is None, [(r.budget, r.loss) for r in sr]))
    return ([
        lc,
    ])
...
class json_result_logger(object):
        ...
        try:
            ...
            if overwrite:
                ...
                raise FileExistsError(
                    'The file %s already exists.' % self.config_fn)
        except:
            raise
    ...
    def __call__(self, job):
        ...
        with open(self.results_fn, 'a') as fh:
            fh.write(
                json.dumps([
                    job.id, job.kwargs['budget'], job.timestamps, job.result,
                    job.exception
                ]))
            fh.write("\n")
...
def logged_results_to_HBS_result(directory):
    ...
    with open(os.path.join(directory, 'configs.json')) as fh:
        for line in fh:
            ...
            data[tuple(config_id)] = Datum(
                config=config, config_info=config_info)

    with open(os.path.join(directory, 'results.json')) as fh:
        for line in fh:
            config_id, budget, time_stamps, result, exception = json.loads(
                line)
    ...
    HB_config = {
        'eta': None
        if len(budget_list) < 2 else budget_list[1] / budget_list[0],
...
class Result(object):
    ...
    def get_incumbent_trajectory(self,
        ...
        if not all_budgets:
            all_runs = list(
                filter(lambda r: r.budget == res.HB_config['max_budget'],
                       all_runs))
        ...
        for b in d.results.keys():
            try:
                ...
                if d.results[b] is None:
                    r = Run(config_id, b, None, None, d.time_stamps[b],
                            err_logs)
                else:
                    r = Run(config_id, b, d.results[b]['loss'],
                            d.results[b]['info'], d.time_stamps[b], err_logs)
                runs.append(r)
            except:
                raise
    ...
    def get_learning_curves(self,
                            lc_extractor=extract_HBS_learning_curves,
                            config_ids=None):
        """
        ...
        for k, v in new_dict.items():
            for kk, vv in v.time_stamps.items():
                for kkk, vvv in vv.items():
                    new_dict[k].time_stamps[kk][
                        kkk] = vvv - self.HB_config['time_ref']
