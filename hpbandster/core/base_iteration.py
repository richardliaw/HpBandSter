...
class BaseIteration(object):
    ...
    def add_configuration(self, config=None, config_info={}):
        ...
        config_id = (self.HPB_iter, self.stage,
    ...
    def get_next_run(self):
        ...
        if (self.actual_num_configs[self.stage] <
                self.num_configs[self.stage]):
    ...
    def _advance_to_next_stage(self, config_ids, losses):
        ...
        raise NotImplementedError(
            '_advance_to_next_stage not implemented for %s' %
            type(self).__name__)
    ...
    def process_results(self):
        ...
        config_ids = list(
            filter(lambda cid: self.data[cid].status == 'REVIEW',
                   self.data.keys()))
        ...
        losses = np.array(
            [self.data[cid].results[budget]['loss'] for cid in config_ids])
    ...
    def finish_up(self):
        ...
        for k, v in self.data.items():
            assert v.status in ['TERMINATED', 'REVIEW', 'CRASHED'
