...
class Master(object):
    ...
    def run(
        ...
        if self.time_ref is None:
            ...
            self.logger.info(
                'HBMASTER: starting run at %s' % (str(self.time_ref)))
        ...
        return Result([copy.deepcopy(i.data)
                       for i in self.iterations] + ws_data, self.config)

    def adjust_queue_size(self, number_of_workers=None):

        self.logger.debug('HBMASTER: number of workers changed to %s' %
        ...
        with self.thread_cond:
            ...
            if self.dynamic_queue_size:
                ...
                self.logger.info('HBMASTER: adjusted queue size to %s' % str(
    ...
    def _submit_job(self, config_id, config, budget):
		...
		"""
        self.logger.debug('HBMASTER: trying submitting job %s to dispatcher' %
                          str(config_id))
        with self.thread_cond:
            self.logger.debug(
