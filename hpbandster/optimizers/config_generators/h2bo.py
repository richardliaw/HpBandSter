...
class H2BO(base_config_generator):
    ...
    def get_config(self, budget):
        ...
        if sample is None:
            try:
                ...
                samples = self.kde_models[budget]['good'].sample(
                    self.num_samples)
    ...
    def new_result(self, job, update_model=True):
        ...
        self.kde_models[budget]['bad'].fit(
            train_data_bad, bw_estimator=self.bw_estimator)
        self.kde_models[budget]['good'].fit(
            train_data_good, bw_estimator=self.bw_estimator)
