...
class Gaussian(BaseKernel):
    def _compute_weights(self):
        ...
        for i, d in enumerate(self.data):
            weights[i] = 2. / (erf(
                (1 - d) /
                (np.sqrt(2) * self.bw)) + erf(d / (np.sqrt(2) * self.bw)))
    ...
    def sample(self, sample_indices=None, num_samples=1):
        ...
        samples += delta
        oob_idx = np.argwhere(np.logical_or(samples > 1,
                                            samples < 0)).flatten()
        ...
        while len(oob_idx) > 0:
            ...
            oob_idx = oob_idx[np.argwhere(
                np.logical_or(samples[oob_idx] > 1,
...
class WangRyzinInteger(BaseKernel):
    ...
    def __call__(self, x_test):
        ...
        pdfs[idx] = (1 - self.bw)
        pdfs[~idx] = 0.5 * (1 - self.bw) * np.power(
            self.bw,
    ...
    def sample(self, sample_indices=None, num_samples=1):
        ...
        possible_steps = np.arange(-self.num_values + 1,
