...
class MultivariateKDE(object):
    ...
    def fit(self,
        ...
        if not efficient_bw_estimation or bw_estimator == 'scott':
            ...
            self.bandwidths = 1.059 * np.minimum(sigmas, IQRs) * np.power(
                self.data.shape[0], -0.2)
            # crop bandwidths for categorical parameters
            self.bandwidths = np.clip(self.bandwidths, self.min_bandwidth,
                                      self.bw_clip)
    ...
    def pdf(self, x_test):
        ...
        if self.fully_dimensional:
            ...
            pdfs = np.sum(
                np.prod(pdfs, axis=-1) * self.weights[None, :], axis=-1)
        else:
