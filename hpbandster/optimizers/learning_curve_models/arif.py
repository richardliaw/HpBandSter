...
class ARIF(lcm_base):
    ...
    def invert_differencing(self, initial_part, differenced_rest, order=None):
        ...
        for s in starting_points[::-1]:
            actual_predictions = np.cumsum(np.hstack([s,
    ...
    def extend_partial(self, obs_losses, num_steps, config=None):
        ...
        prediction = self.invert_differencing(obs_losses,
                                              d_losses[-num_steps:])
...
if __name__ == "__main__":
    ...
    sys.path.append(
        "/home/sfalkner/repositories/bitbucket/learning_curve_prediction")
    ...
    data = load_configs(
        "/home/sfalkner/repositories/bitbucket/learning_curve_prediction/data/fc_net_mnist",
        1024)
    ...
    learning_curves = [
        lc[:np.random.randint(lc.shape[0] - 8) + 8] for lc in data[1]
    ]
    ...
    reconstruction = lc_model.invert_differencing(
        random_sequence[:1 + test_order], tmp, order=test_order)
    ...
    for i in range(16):
        ...
        plt.plot(
            range(
                len(learning_curves[i]),
                len(learning_curves[i]) + len(pred)), pred, '--')
    plt.show()
