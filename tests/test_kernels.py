...
class TestGaussian(unittest.TestCase):
    ...
    def test_values(self):
        ...
        for bw in [1e-3, 1e-2, 1e-1, 1]:
            sm_values = sm_kernels.gaussian(bw, self.x_train[:, None],
                                            self.x_test[None, :])
            hp_kernel = hp_kernels.Gaussian(
                data=self.x_train, bandwidth=bw, fix_boundary=False)
    ...
    def test_pdf_boundary_simple(self):
        ...
        for bw in [1e-3, 1e-2, 1e-1]:
            ...
            hp_kernel1 = hp_kernels.Gaussian(
                data=self.x_train, bandwidth=bw, fix_boundary=False)
            hp_kernel2 = hp_kernels.Gaussian(
                data=self.x_train, bandwidth=bw, fix_boundary=True)
...
class Test1dCategorical(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.x_train = np.array(
            [c.get_array() for c in x_train_confs]).squeeze()
    ...
    def test_values(self):
        for bw in [1e-3, 1e-2, 1e-1, 1]:
            ...
            for x in self.x_test:
                sm_values.append(
                    sm_kernels.aitchison_aitken(bw, self.x_train, x))
            sm_values = np.array(sm_values)
            hp_kernel = hp_kernels.AitchisonAitken(
                data=self.x_train,
                bandwidth=bw,
                num_values=len(
                    self.configspace.get_hyperparameters()[0].choices))
            hp_values = hp_kernel(self.x_test)
            self.assertTrue(
                np.allclose(hp_values.T, sm_values.squeeze(), 1e-4))
...
class Test1dInteger(unittest.TestCase):
    ...
    @unittest.skipIf(rapid_development,
                     "test skipped to accelerate developing new tests")
    def test_values(self):
        n = self.configspace.get_hyperparameters(
        )[0].upper - self.configspace.get_hyperparameters()[0].lower + 1
        ...
        for bw in [1e-3, 1e-2, 1e-1, 0.99]:
            ...
            sm_values = np.array([
                sm_kernels.wang_ryzin(bw, sm_x_train[:, None], x)
                for x in sm_x_test
            ]).squeeze()
            hp_kernel = hp_kernels.WangRyzinInteger(
                data=self.x_train,
                bandwidth=bw,
                num_values=n,
                fix_boundary=False)
    ...
    def test_sample(self):
        ...
        for bw in [1e-1, 5e-1, 0.99]:
            ...
            samples = hp_kernel.sample(num_samples=num_samples)
            phat1, x = np.histogram(
                samples, normed=True, bins=[0, 0.2, .4, .6, .8, 1.])
...
class Test1dOrdinal(unittest.TestCase):
    ...
    def test_values(self):
        ...
        for bw in [1e-3, 1e-2, 1e-1, 1]:
            sm_values = np.array([
                sm_kernels.wang_ryzin(bw, self.x_train[:, None], x)
                for x in self.x_test
            ])
            hp_kernel = hp_kernels.WangRyzinOrdinal(
                data=self.x_train, bandwidth=bw, fix_boundary=False)
            hp_values = hp_kernel(self.x_test)
            self.assertTrue(np.allclose(hp_values.T, sm_values, 1e-4))
    ...
    def test_pdf_boundary_simple(self):
        ...
        for bw in [1e-3, 1e-2]:
            ...
            hp_kernel1 = hp_kernels.WangRyzinOrdinal(
                data=self.x_train,
                bandwidth=bw,
                num_values=4,
                fix_boundary=False)
            hp_kernel2 = hp_kernels.WangRyzinOrdinal(
                data=self.x_train,
                bandwidth=bw,
                num_values=4,
                fix_boundary=True)
    ...
    def test_sample(self):
        ...
        for bw in [1e-1, 5e-1, 0.99]:
            ...
            samples = hp_kernel.sample(num_samples=num_samples)
            phat1, x = np.histogram(
                samples, normed=True, bins=[-0.5, 0.5, 1.5, 2.5, 3.5])
            phat2 = hp_kernel((x[1:] + x[:-1]) / 2).mean(axis=0)
            for p1, p2 in zip(phat1, phat2):
                self.assertAlmostEqual(p1, p2, delta=5e-2)
