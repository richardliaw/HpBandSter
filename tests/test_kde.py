...
class Base1dTest(object):
    ...
    def setUp(self):
        ...
        self.sm_kde = sm.nonparametric.KDEMultivariate(
            data=self.sm_x_train, var_type=self.var_types, bw='cv_ml')
        self.hp_kde_full = MultivariateKDE(
            self.configspace, fully_dimensional=True, fix_boundary=False)
        self.hp_kde_factor = MultivariateKDE(
            self.configspace, fully_dimensional=False, fix_boundary=False)
    ...
    @unittest.skipIf(rapid_development,
                     "test skipped to accelerate developing new tests")
    def test_bandwidths_estimation(self):
        ...
        self.assertAlmostEqual(
            self.sm_kde.bw[0], self.hp_kde_full.bandwidths[0], delta=2e-3)
        self.assertAlmostEqual(
            self.sm_kde.bw[0], self.hp_kde_factor.bandwidths[0], delta=2e-3)
    ...
    def test_loo_likelihood(self):
        for bw in np.logspace(-1, -0.1, 5):
            ...
            self.assertAlmostEqual(
                sm_ll, n * (hp_full_ll - np.log(n - 1)), delta=delta)
            self.assertAlmostEqual(
                sm_ll, n * (hp_factor_ll - np.log(n - 1)), delta=delta)
...
class BaseNdTest(object):
    ...
    def test_bandwidths_estimation(self):
        ...
        for d in range(len(self.var_types)):
            self.assertAlmostEqual(
                self.sm_kde.bw[d], self.hp_kde_full.bandwidths[d], delta=5e-2)
            self.assertAlmostEqual(
                self.sm_1d_kdes[d].bw[0],
    ...
    def test_pdfs(self):
        for bw in np.logspace(-0.5, -0.1, 5):
            ...
            self.hp_kde_full.set_bandwidths(
                np.array([bw] * len(self.var_types)))
            self.hp_kde_factor.set_bandwidths(
                np.array([bw] * len(self.var_types)))
    ...
    def test_loo_likelihood(self):
        for bw in np.logspace(-1, -0.1, 5):
            ...
            sm_full_ll = self.sm_kde.loo_likelihood(
                bw=np.array([bw] * len(self.var_types)), func=np.log)
            hp_full_ll = self.hp_kde_full.loo_negloglikelihood()
            hp_factor_ll = self.hp_kde_factor.loo_negloglikelihood()
            ...
            for i, kde in enumerate(self.sm_1d_kdes):
                kde.bw = np.array([bw])
                sm_factor_ll.append(
                    kde.loo_likelihood(bw=np.array([bw]), func=np.log))
